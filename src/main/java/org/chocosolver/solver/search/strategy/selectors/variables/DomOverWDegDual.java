...
public class DomOverWDegDual extends AbstractStrategy<IntVar> implements IMonitorContradiction, IMonitorUpBranch {
...
//    protected IntMap p2w;
     ...
     */
    public DomOverWDegDual(IntVar[] variables, long seed, IntValueSelector valueSelector) {
        super(variables);
        corr=new double[vars.length][vars.length];
...
//                ss[p2i.get(p.getId())][v2i.get(vars[idx].getId())]+= 1.0/(a*(1+vars[idx].getDomainSize()));
//                ss[p2i.get(p.getId())][v2i.get(vars[idx].getId())]+= 1.0+corr[vIndex][v2i.get(vars[idx].getId())];
//                ss[p2i.get(p.getId())][v2i.get(vars[idx].getId())]+= 1.0+corr[vIndex][v2i.get(vars[idx].getId())]/(a*(1+vars[idx].getDomainSize()));
                ss[p2i.get(p.getId())][v2i.get(vars[idx].getId())]+= 1.0/(corrWeight(vars[idx])*(a*(1+vars[idx].getDomainSize())));
            }
            conflict=false;
        }
    }
