...
public class AlgoAllDiffAC_Fastbit1 {
    ...
    // 与变量相连的值
    private NaiveBitSet[] varToVal;
    // 与值相连的变量
    private NaiveBitSet[] valToVar;

    // 区分变量
    private NaiveBitSet varDis;
    // 区分值
    private NaiveBitSet valDis;

    // 未搜索变量
    private NaiveBitSet varNotSearch;
    // 未搜索值
    private NaiveBitSet valNotSearch;

    // 待搜索变量
    private NaiveBitSet varNeedSearch;
    // 待搜索值
    private NaiveBitSet valNeedSearch;

    // 待搜索变量
    private NaiveBitSet varRecord;
    // 待搜索值
    private NaiveBitSet valRecord;
    ...
    public AlgoAllDiffAC_Fastbit1(IntVar[] variables, ICause cause) {
        ...
        varToVal = new NaiveBitSet[n];
        valToVar = new NaiveBitSet[numValue];

        varDis = new NaiveBitSet(n);
        valDis = new NaiveBitSet(numValue);

        varNotSearch = new NaiveBitSet(n);
        valNotSearch = new NaiveBitSet(numValue);

        varNeedSearch = new NaiveBitSet(n);
        valNeedSearch = new NaiveBitSet(numValue);

        varRecord = new NaiveBitSet(n);
        valRecord = new NaiveBitSet(numValue);

        for (int i = 0; i < n; ++i) {
            varEdge[i] = new NaiveBitSet(numBit);
            varToVal[i] = new NaiveBitSet(numValue);
        }
        for (int i = 0; i < numValue; ++i) {
            valEdge[i] = new NaiveBitSet(numBit);
            valToVar[i] = new NaiveBitSet(n);
        }
    ...
    private void findMaximumMatching() throws ContradictionException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (k = v.getLB(); k <= ub; k = v.nextValue(k)) {
                ...
                // 更新leftEdge varToVal valToVar
                int valIdx = j - n;
                leftEdge.set(i * numValue + valIdx);
                varToVal[i].set(valIdx);
                valToVar[valIdx].set(i);
            }
        }
...
//                        System.out.println(v.getName() + " remove " + k);
    ...
    private boolean checkSCC(int edgeIdx) {
        // 先根据是否是匹配边初始化
        int matchedValNewIdx = edgeIdx % numValue;
        int varIdx = edgeIdx / numValue;
        searchEdge.clear();
        varNeedSearch.clear();
        varNotSearch.set(varDis);
        if (matchedEdge.get(edgeIdx)) { // 如果edge是匹配边
            searchEdge.addIntersection(valEdge[matchedValNewIdx], leftEdge);
            searchEdge.clear(edgeIdx);
            varNeedSearch.addIntersection(valToVar[matchedValNewIdx], varNotSearch);
            varNeedSearch.clear(varIdx);
        } else { // 如果edge是非匹配边
            searchEdge.set(edgeIdx);
            varNeedSearch.set(varIdx);
        }
//                Measurer.propNum++;
        // 开始搜索
        long startTime = System.nanoTime();
        // 头部扩展，匹配变量
        while (!varNeedSearch.isEmpty()) {
            varNotSearch.clear(varNeedSearch);
            varRecord.clear();
            for (varIdx = varNeedSearch.nextSetBit(0); varIdx > 0; varIdx = varNeedSearch.nextSetBit(varIdx + 1)) {
                searchEdge.set(varMatchedEdge[varIdx]);
                int valNewIdx = matching[varIdx] - n;
                if (valNewIdx == matchedValNewIdx) {
                    Measurer.checkSCCTime += System.nanoTime() - startTime;
                    return true;
                }
                // 把与匹配值相连的边和变量并入
                searchEdge.addIntersection(valEdge[valNewIdx], leftEdge);
                varRecord.addIntersection(valToVar[matchedValNewIdx], varNotSearch);
            }
            varNeedSearch.set(varRecord);
        }
        Measurer.checkSCCTime += System.nanoTime() - startTime;
        return false;
    }
