...
//        sVal.clear();
        ...
        for (int x = 0; x < arity; x++) {
            ...
            if (v.getDomainSize() == 1) {
                // var2Val[x] 是当前变量的匹配值，可能是-1，判断是否有效

//                int valueIndex = val2Idx.get(v.getValue());
//                int destValIndex = v.getValue();
//                int oriVar = var2Val[destValIndex];
//
//                if (var2Val[x] == -1) {
//                    // 记住的那个匹配已经无效
//                    var2Val[x] = destValIndex;
//                    val2Var[destValIndex] = x;
//
//
//                } else {
//                    if (!v.contains(var2Val[x])) {
//
//                        // 这个可以不要 因为不会参与匹配或传播
//                        varMask[x].clear();
//                        varMask[x].set(valueIndex);
//                        oldValIdx = var2Val[x];
//                        if (val2Var[valueIndex] == -1 && oldValIdx != -1) {
//                            // 释出旧匹配值
//                            val2Var[oldValIdx] = -1;
...
//                    }
//                }
                int offset_value = v.getValue();
                if (val2Var[offset_value] == -1) {
                    val2Var[offset_value] = x;
                    var2Val[x] = offset_value;
                    freeNode.clear(offset_value);
                }
