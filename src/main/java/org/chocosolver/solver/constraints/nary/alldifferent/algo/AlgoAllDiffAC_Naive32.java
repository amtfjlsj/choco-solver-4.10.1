...
// arity小于32 采用此propagator 精简了临接矩阵的数据结构
public class AlgoAllDiffAC_Naive32 {
...
//        }
        //////////用int做为Mask//////////
        lastMask = WORD_MASK >>> (BITS_PER_WORD - arity);
        ////////////////////////////////
...
//        }
        ...
        for (int i = nextSetBit(gammaFrontier, 0);
             i != BITS_PER_WORD; i = i = nextSetBit(gammaFrontier, 0)) {
            // !! 这里可以将Extended改成Frontier，只记录前沿，记录方法是三个BitSet比较，
            // frontier 扩展，从valMask中去掉gammaMask已记录的变量
//            gammaFrontier.orAfterMinus(valMask[var2Val[i]], gammaMask);
...
//        if (graphLinkedMatrix[varIdx].get(val2Var[valIdx])) {
        if ((graphLinkedMatrix[varIdx] & 1L << val2Var[valIdx]) != 0) {
            return true;
        }
